class Room {
    constructor(streamerId) {
        ...
        this.viewerCount = 0;
        this.startTime = new Date(); // Start time of the room
    }
    ...
    addProducer(kind, producer) {
        if (this.streamerProducers.has(kind)) {
            console.warn(
                `Producer of kind '${kind}' already exists for this room`
            );
            return;
        }
        this.streamerProducers.set(kind, producer);
        console.log(
            `Producer of kind '${kind}' added for streamer ${this.streamerId}`
        );
    }
    ...
    removeProducer(kind) {
        ...
        console.log(
            `Producer of kind '${kind}' removed for streamer ${this.streamerId}`
        );
    }
    ...
    addViewer(viewerId, transport) {
        ...
        console.log(
            `Viewer '${viewerId}' added to the room for streamer ${this.streamerId}`
        );
    }
    ...
    removeViewer(viewerId) {
        ...
        console.log(
            `Viewer '${viewerId}' removed from the room for streamer ${this.streamerId}`
        );
    }
